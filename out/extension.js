            ...
            messages: [
                ...
                { role: "assistant", content: "the example step definition javascript file is {step} = " + config_json_1.default.few_shot_assistant_step_code },
                {
                    role: "user", content: "for {feature} = " + inputText + "lets think step by step."
                        ...
                        + "The example syntax is - const LoginPage = require('../pages/login.page');"
                        + "Step 2: Strictly write multiple imports as separate lines if using more than one page object class. "
                        + "For example, this is incorrect - import { calcPage, compPage } from '@pages'; "
                        + "and the correct syntax is - import { calcPage } from '@pages/CalcPage'; import { compPage } from '@pages/CompPage'; "
        ...
        messages: [
            ...
            { role: "assistant", content: "the page object class implementation is {page} = " + config_json_1.default.few_shot_assistant_page_code },
            {
                role: "user", content: "lets think step by step." +
                    "Step 1: Define all Web element selectors in a dedicated `locators` object at the top of the class file." +
                    "Step 2: Implement every UI interaction method using Playwright APIs with async/await syntax (e.g., `page.fill`, `page.click`, `page.waitForSelector`)." +
                    "Step 3: Each method must contain full logic with required waits and validations (e.g., `waitForSelector`, `isVisible`, `innerText`)." +
                    "Step 4: Do not leave any method partially implemented or stubbed. If the behavior is not provided, assume and write the full logic." +
                    "Step 5: Use realistic, commonly expected behaviors where application specifics are missing (e.g., error messages, navigation verification, etc.)." +
                    "Step 6: Avoid leaving any TODOs or placeholders—complete all logic for element interaction and verification methods." +
                    "Step 7: Ensure that a Page Object Class is created and exported correctly (e.g., `module.exports = { LoginPage };`) and that it matches imports in the step definition file." +
                    "Step 8: Ensure that the Playwright code is syntactically valid and free from JavaScript errors." +
...
async function activate(context) {
    ...
    console.log('Extension "Playwright Copilot" is now active!');
    // Command to store the API key
    const setApiKeyCommand = vscode.commands.registerCommand('pwc.setApiKey', async () => {
    ...
    const processtest = vscode.commands.registerCommand('pwc.processTest', () => {
...
function getWebviewContent() {
  ...
  <html>
	...
	<script type="text/javascript">
		...
		function clearAll() {
			document.getElementById("txt").value = "Enter BDD scenario here to generate Playwright code for web application testing.";
		...
		function sampleScenario(){
			document.getElementById("txt").value = "Feature: SauceDemo Login\\n" 
	...
	<form>
  		...
  		<table style="width: 100%;">
    		<tr>
        		<td colspan=4>
					<h1 class="fancy">Playwright Copilot</h1>
    		...
    		<tr>
        		<th colspan=4>
					<textarea id="txt" rows="14" cols="105" align="left">Enter BDD scenario here to generate Playwright code for web application testing.</textarea>
				</th>
    		</tr>
			<tr><td colspan=4>&nbsp;</td></tr>
			<tr>
